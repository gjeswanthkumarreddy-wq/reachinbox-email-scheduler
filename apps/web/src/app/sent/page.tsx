...
export default function SentPage() {
  ...
  useEffect(()=>{
    setLoading(true);
    api.emails.sent(page).then(setData).catch(e=>setError(e.message)).finally(()=>setLoading(false));
  },[page]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {loading?<LoadingSpinner/>:data?.jobs?.length===0?<EmptyState title="No sent emails yet" message="Sent emails appear here once the worker processes your campaigns."/>:(
          <>
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>{['Recipient','Campaign','Status','Sent At','Preview'].map(h=><th key={h} className="text-left px-4 py-3 text-gray-600 font-medium">{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {data?.jobs?.map((j:any)=>(
                    <tr key={j.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-500 text-xs">{j.sent_at?new Date(j.sent_at).toLocaleString():'—'}</td>
                      <td className="px-4 py-3">{j.message_id&&<a href={'https://ethereal.email/message/'+j.message_id} target="_blank" rel="noreferrer" className="text-blue-600 text-xs hover:underline">View</a>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
